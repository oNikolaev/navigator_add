...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ЗначениеНастройки = Настройки.Получить("ОбластьПоискаИмя");
	
	Если Не (ЗначениеНастройки = Неопределено) Тогда
		Элементы.КомандаОбластьПоискаИмя.Пометка = ЗначениеНастройки;
	КонецЕсли;
	
	//
	
	ЗначениеНастройки = Настройки.Получить("ОбластьПоискаСиноним");
	
	Если Не (ЗначениеНастройки = Неопределено) Тогда
		Элементы.КомандаОбластьПоискаСиноним.Пометка = ЗначениеНастройки;
	КонецЕсли;
	
	//
	
	ЗначениеНастройки = Настройки.Получить("ОбластьПоискаКомментарий");
	...
	ЗначениеНастройки = Настройки.Получить("СтрокаПоискаПоСтруктуреМетаданных");
	
	СтрокаПоискаУказана = ЗначениеЗаполнено(ЗначениеНастройки);
...
Процедура КомандаСписокЭлементовОМРедактированиеЗаписи(Элемент)
	...
	ИмяСпискаЭлементов = НВ_ПреобразовательКлиентСервер.ИмяСпискаПоИмениКоманды(Элемент.Имя);
	...
	ПараметрыЗаписи = НВ_ПреобразовательКлиентСервер.ПараметрыТаблицыПоИмениКоманды(Элемент.Имя);
	...
	Если (НВ_ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеЗаписи, "Регистратор")) Тогда
		ПараметрыОткрытия.Вставить("ОбъектБазы", ДанныеЗаписи.Регистратор);
	Иначе
		ПараметрыОткрытия.Вставить("ОбъектБазы", Неопределено);
	КонецЕсли;
