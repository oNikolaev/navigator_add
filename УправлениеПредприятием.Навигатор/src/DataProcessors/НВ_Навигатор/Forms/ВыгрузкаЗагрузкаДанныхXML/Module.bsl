...
Процедура КомандаПрименитьИзмененияXMLДляОбъектаБазы(Команда)
	...
	ПрименитьИзмененияXMLДляОбъектаБазы();
...
&НаКлиенте
Процедура ПрименитьИзмененияXMLДляОбъектаБазы()
	
	#Область Предусловия
	Если (Не ЗначениеЗаполнено(ДанныеXML)) Тогда
		Возврат;
	КонецЕсли;
	#КонецОбласти
...
Функция НаборИзЗаписиРегистраСведений(ЗаписьРегистраСведений)
	...
	// TODO: Доделать функцию, а то сейчас получается записи регистра не выгружаются.
	...
	Для Каждого Измерение Из МетаданныеРегистра.Измерения Цикл
		НаборЗаписей.Отбор[Измерение.Имя].Установить(ЗаписьРегистраСведений[Измерение.Имя]);
	КонецЦикла; 
